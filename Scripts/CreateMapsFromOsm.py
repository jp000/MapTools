...
def fileAge(file_path):
    if not os.path.exists(file_path) or os.path.getsize(file_path) < 268:
        return 36500
...
def mkgmap(inputfilename,
    ...
    cmd = fr'java -Xmx4000m -jar "{javaJar}" -c "{options}" {route} --style-file="{styleDir}" --family-id=1000 --product-id=1 {mapName} --description="{mapDescription}" --output-dir="{outputDir}" {inputfilename} {typeFile}'
...
def mkgmapTest(inputfilename,
    ...
    cmd = fr'java -Xmx4000m -jar "{javaJar}" -c "{options}" {route} --family-id=1000 --product-id=1 {mapName} --description="{mapDescription}" --output-dir="{outputDir}" {inputfilename}'
...
def mkgsplit(inputfilename,
    ...
    if mapId == 46600000:  # special for thailand
        maxNodes = 1600000
    cmd = fr'java -Xmx4000m -jar "{javaJar}" --max-nodes={maxNodes} --mapid={mapId} --description={description} --output-dir="{outputDir}" "{inputfilename}"'
    print(f'Execute {cmd}')
    p = subprocess.Popen(shlex.split(cmd), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    r = p.communicate()
    if p.returncode == 0:
...
def make_Bbox1(nbr, minLat=-90.0, minLon=-180.0, maxLat=90.0, maxLon=180.0):
...
def processWithBbox(nbr, query, minLat, minLon, maxLat, maxLon, outputName, startIndex, server):
    ...
    for i in range(0, len(nodes)):
        ...
        filenameIndex += 1
        if doOverpassReq(n, query, server=server, bbox=swapNode(nodes[i])):
...
if __name__ == '__main__':
    ...
    if False:
        outputName = 'X'
        outputnames, idx = processWithBbox(2,
                                           '(node[place](if: number(t["population"]) >= 400);out body;',
                                           minLat=46.0, minLon=7.0, maxLat=47.0, maxLon=8.0,
                                           outputName='X', startIndex=1, server=servers[0])
    ...
    if False:
        ...
        q1 = """
            ...
            node[shop=bicycle](area.ch)(around:100);>;
            out body;
        """
